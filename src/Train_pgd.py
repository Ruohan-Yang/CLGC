...
def train_phase(model, train_loader, valid_loader, args, log, best_valid_dir):
    ...
    for epoch in trange(args.epochs, desc="Training"):
        ...
        for data in train_loader:
            ...
            prediction_outs = model(network_labels, left_nodes, right_nodes)

            whole_loss = criterion(prediction_outs, link_labels)
...
def run_model(train_loader, valid_loader, test_loader, model, args):
    ...
    best_valid_dir = 'save/' + args.dataset + '_ablation_best_model.pth'
    if args.onlyTest and os.path.exists(best_valid_dir):
        print('onlyTest, best_model exists')
        log.write('onlyTest\n')
    else:
        best_valid_dir = train_phase(model, train_loader, valid_loader, args, log, best_valid_dir)
