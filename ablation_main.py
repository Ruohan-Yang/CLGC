...
from src.SUPPORT_pgd import Model_Net
from src.Train_pgd import run_model
import warnings
...
def get_args():
    ...
    parser.add_argument('--model', type=str, default='SUPPORT_pgd', help='name of model')
    ...
    parser.add_argument('--EarlyStop', action='store_false', help='Default is false. If true, enable early stopping strategy')
    parser.add_argument('--patience', type=int, default=5, help='early stop')
    parser.add_argument('--best_metric', type=str, default='aupr', help='Primary evaluation metric used for early stopping (e.g., auc, ap, aupr).')

    parser.add_argument('--gcn_type', type=str, default='JK_GCN', help='e.g., GCN, JK_GCN')
    parser.add_argument('--gcn_layer', type=int, default=3, help='e.g., 2, 3')
...
def run_support_ablation():
    ...
    print('***************************')
    args.log = log_path + args.dataset + '-ablation-Result-R1.txt'
    print(args)
